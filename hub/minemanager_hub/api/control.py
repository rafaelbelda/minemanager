...
def _agent_and_spec(instance_id: str) -> tuple[AgentConnection, dict]:
    ...
    The spec (including a just-in-time decrypted RCON password) is attached to
    every instance-scoped command so the agent stays stateless about config.
    """
    with session_scope() as db:
        ...
        spec = InstanceSpec(
            ...
            rcon_password=_lookup_secret(db, instance_id, "rcon_password"),
